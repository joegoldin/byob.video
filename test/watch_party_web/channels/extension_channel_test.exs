defmodule WatchPartyWeb.ExtensionChannelTest do
  use ExUnit.Case, async: false

  alias WatchPartyWeb.{ExtensionSocket, ExtensionChannel}
  alias WatchParty.RoomServer

  setup do
    room_id = "ext_test_#{:erlang.unique_integer([:positive])}"
    {:ok, _pid} = WatchParty.RoomManager.ensure_room(room_id)
  ...
  defp connect(module, params, connect_info) do
    module.connect(params, %Phoenix.Socket{
      endpoint: WatchPartyWeb.Endpoint,
      handler: module,
      transport: :websocket,
  ...
  defp subscribe_and_join(socket, channel, topic, payload) do
    ...
    case channel.join(topic, payload, socket) do
      {:ok, reply, socket} ->
        ...
        Phoenix.PubSub.subscribe(WatchParty.PubSub, "room:#{extract_room_id(topic)}")
        {:ok, reply, socket}
  ...
  describe "handle_in" do
    ...
    test "video:pause broadcasts sync_pause", %{socket: socket, room_id: room_id} do
      ...
      pid = GenServer.whereis({:via, Registry, {WatchParty.RoomRegistry, room_id}})
